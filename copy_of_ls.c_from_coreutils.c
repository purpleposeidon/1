...
static void print_name_with_quoting (const struct fileinfo *f,
                                       bool symlink_target,
...
#define DIRED_PUTCHAR(t, c) do {dired_pos += l1_putchar ((t), (c));} while (0)

/* Write S to STDATR and increment DIRED_POS by the number of std chars written.  */
#define DIRED_PUTS(t, s) \
    do {dired_pos += l1_puts (t, s);} while (0)
/* Like DIRED_PUTS, but for use when S is a literal string.  */
#define DIRED_PUTS_LITERAL_FORMAT(s) \
    do {l1_puts (L1_FORMAT, s); dired_pos += sizeof (s) - 1;} while (0)

#define DIRED_INDENT()							\
    do									\
      {									\
        if (dired)							\
          DIRED_PUTS_LITERAL_FORMAT ("  ");				\
      }									\
    while (0)
...
static void
dired_dump_obstack (const char *field_name, const char *prefix, struct obstack *os)
{
  size_t n_pos;
  ...
  if (n_pos > 0)
    {
      ...
      l1_puts(L1_FIELD_NAME, field_name);
      l1_puts(L1_FIELD_TEXT, prefix);
      for (size_t i = 0; i < n_pos; i++)
        l1_printf (L1_FIELD_TEXT, " %lu", (unsigned long int) pos[i]);
      l1_putchar(L1_FORMAT, '\n');
    }
  // FIXME: This is weird, in the same way that "total" is weird.
}
  ...
  if (dired)
    {
      ...
      dired_dump_obstack ("dired", "//DIRED//", &dired_obstack);
      dired_dump_obstack ("subdired", "//SUBDIRED//", &subdired_obstack);
      l1_printf (L1_FORMAT, "//DIRED-OPTIONS// --quoting-style=%s\n",
  ...
  if (format == long_format || print_block_size)
    {
      ...
      DIRED_INDENT ();
      l1_puts(L1_FIELD_NAME, "total"); // FIXME: 'total' problem
      dired_pos += l1_printp(L1_PFORMAT "%s ", _("total"));
      p = human_readable (total_blocks, buf, human_output_opts,
                          ST_NBLOCKSIZE, output_block_size);
      DIRED_PUTS (L1_FIELD_TEXT, p);
      DIRED_PUTCHAR (L1_FORMAT, '\n');
    }
...
// FIXME: don't allow combining these, somehow...?
static const char *stat_failed_name = "?";

static void
format_user_or_group (char const *name_label, char const *name, char const *id_label, unsigned long int id, int width)
{
  if (name)
    {
      if (name != stat_failed_name)
        {
          /* hidden numeric id */
          l1_printp(
              L1_PFIELD_NAME "%s"
      ...
      // ?? else l1_printf(L1_FIELD_TYPE, "errored");

      l1_field_name(name_label);
      DIRED_PUTS (L1_FIELD_TEXT, name);
      int width_gap = width - mbswidth (name, 0);
      int pad = MAX (0, width_gap);
      l1_printf(L1_FORMAT, "%*s", pad + 1, "");
      dired_pos += pad + 1;
    }
  else
    {
...
static void
format_user (char const *name_label, char const *id_label, uid_t u, int width, bool stat_ok)
{
  char const *name = ! stat_ok ? stat_failed_name :
                     (numeric_ids ? NULL : getuser (u));
  format_user_or_group (name_label, name, id_label, u, width);
}

/* Likewise, for groups.  */

static void
format_group (char const *group_label, char const *id_label, gid_t g, int width, bool stat_ok)
{
  char const *name = ! stat_ok ? stat_failed_name :
                     (numeric_ids ? NULL : getgroup (g));
  format_user_or_group (group_label, name, id_label, g, width);
}
...
{
  char modebuf[12];
  ssize_t col_start = dired_pos;
  struct timespec when_timespec;
  struct tm when_local;
  bool btime_ok = true;

  DIRED_INDENT ();
  // FIXME: l1_printp filetype
  ...
  if (print_inode)
    {
      ...
      char *inode = format_inode (hbuf, sizeof hbuf, f);

      if (*inode == '?')
        dired_pos += l1_printp (
                L1_PFORMAT "%*s ",
                inode_number_width - strlen(inode), inode);
      else
        {
          l1_puts (L1_FIELD_NAME, "inode");
          int spaces = inode_number_width - strlen(inode);
          if (spaces > 0) {
            dired_pos += l1_printp (
                  L1_PFORMAT "%*s",
                  spaces, "");
  ...
  if (print_block_size)
    {
      ...
      dired_pos += l1_printp (
              L1_PFIELD_NAME "block_size"
              L1_PFORMAT "%*s"
              L1_PFIELD_TEXT "%s"
              L1_PFORMAT " ",
              block_size_width - mbswidth (blocks, 0), "",
              blocks);
    }
  ...
  {
    char hbuf[INT_BUFSIZE_BOUND (uintmax_t)];
    const char *val = ! f->stat_ok ? "?" : umaxtostr (f->stat.st_nlink, hbuf);
    int pad = nlink_width - strlen(val);
    dired_pos += l1_printp (
        L1_PFIELD_NAME "mode"
        L1_PFIELD_TEXT "%s" // FIXME: More
        L1_PFIELD_NAME "hardlink_count"
        L1_PFORMAT " %*s"
        L1_PFIELD_TEXT "%s" L1_PFORMAT " ",
        modebuf,
        MAX (0, pad), "",
        val
    );
  ...
  if (print_owner)
    format_user ("user_name", "user_id", f->stat.st_uid, owner_width, f->stat_ok);

  if (print_group)
    format_group ("group_name", "group_id", f->stat.st_gid, group_width, f->stat_ok);

  if (print_author)
    format_user ("author_name", "author_id", f->stat.st_author, author_width, f->stat_ok);

  if (print_scontext)
    format_user_or_group ("security_context", f->scontext, "security_context_id" /* FIXME: ??? */, 0, scontext_width);
  ...
  if (f->stat_ok
      && (S_ISCHR (f->stat.st_mode) || S_ISBLK (f->stat.st_mode)))
    {
      ...
      l1_puts(L1_FIELD_NAME, "major_device_number");
      dired_pos += l1_print_pad(
          L1_FIELD_TEXT,
          major_device_number_width + MAX (0, blanks_width),
          umaxtostr (major (f->stat.st_rdev), majorbuf));
      DIRED_PUTS_LITERAL_FORMAT(", ");
      l1_puts(L1_FIELD_NAME, "minor_device_number");
      dired_pos += l1_print_pad(
          L1_FIELD_TEXT,
          minor_device_number_width,
      ...
      /*
      dired_pos += l1_printp(
          L1_FIELD_NAME "major_device_number"
          L1_FIELD_TEXT "%*s"
          L1_FORMAT ", "
          L1_FIELD_NAME "minor_device_number"
          L1_FIELD_TEXT "%*s"
          L1_FORMAT " ",
          major_device_number_width + MAX (0, blanks_width),
          umaxtostr (major (f->stat.st_rdev), majorbuf),
  ...
  else
    {
      ...
      if (*size == '?')
          ...
          dired_pos += l1_printp (
              ...
              L1_PFORMAT " ",
              f->stat.st_size,
              file_size_width - strlen(size), "", size
          );
    }

  char time_buf[TIME_STAMP_LEN_MAXIMUM + (2 /* FIXME: better safe than sorry?? */)];
  size_t time_buf_len = 0;
  if (f->stat_ok && btime_ok
      && localtime_rz (localtz, &when_timespec.tv_sec, &when_local))
    {
      ...
      time_buf_len = align_nstrftime (time_buf, TIME_STAMP_LEN_MAXIMUM + 1, recent,
                           &when_local, localtz, when_timespec.tv_nsec);
    }

  if (time_buf_len || !*time_buf)
    {
      dired_pos += l1_printp(
          L1_PFIELD_NAME "%s" // FIXME: :eparate padding?
          L1_PFIELD_VALUE "%li" // FIXME format code probably wrong
          L1_PFIELD_TEXT "%s"
          L1_PFORMAT " "
          L1_PFIELD_NAME "%s_ns"
          L1_PFIELD_VALUE "%li",
          time_label, when_timespec.tv_sec, time_buf,
          time_label, when_timespec.tv_nsec
      );
    }
  else
    {
      ...
      dired_pos += l1_printp(
          L1_PFIELD_NAME "%s"
          L1_PFIELD_VALUE "%li" // FIXME format code probably wrong
          L1_PFIELD_TEXT "%*s" // FIXME: separate padding
          L1_PFIELD_NAME "%s_ns"
          L1_PFIELD_VALUE "%li", // FIXME format code probably wrong
          time_label,
  ...
  print_name_with_quoting (f, false, &dired_obstack, dired_pos - col_start);
  ...
  if (f->filetype == symbolic_link)
    {
      if (f->linkname)
        {
          DIRED_PUTS_LITERAL_FORMAT (" -> ");
          print_name_with_quoting (f, true, NULL, dired_pos - col_start);
            ...
            bool allow_pad, struct obstack *stack, char const *absolute_name)
{
  ...
  if (absolute_name)
    {
      // FIXME: url field only gets printed with the --hyperlink option, but it should always print!
      l1_printp (L1_PFIELD_NAME "url" L1_PFIELD_VALUE "file://%s%s%s", hostname, *absolute_name == '/' ? "" : "/", absolute_name);

      if (align_variable_outer_quotes && cwd_some_quoted && ! pad)
        {
          skip_quotes = true;
          l1_putchar (L1_FORMAT, *buf);
        }
      ...
      l1_printf (L1_FORMAT, "\033]8;;file://%s%s%s\a", h, *n == '/' ? "" : "/", n);
      free (h);
      free (n);
    }
  ...
  dired_pos += len;
  {
    // FIXME: push & pop entity; but this happens elsewhere
    l1_field_name("filename");
    l1_field_value(name);
    struct l1_packet p = l1_packet_init(L1_FIELD_TEXT, buf + skip_quotes, len - (skip_quotes * 2));
  ...
  if (absolute_name)
    {
      l1_puts (L1_FORMAT, "\033]8;;\a");
      if (skip_quotes)
        l1_putchar (L1_FORMAT, *(buf + len - 1));
    }
...
print_file_name_and_frills (const struct fileinfo *f, size_t start_col)
{
  ...
  if (print_inode)
    {
      if (f->stat_ok && f->stat.st_ino != NOT_AN_INODE_NUMBER)
        dired_pos += l1_printp(
            L1_PFIELD_NAME "inode"
            L1_PFIELD_VALUE "%i"
            L1_PFIELD_TEXT "%*s ", // FIXME: separate padding
            f->stat.st_ino,
            (format == with_commas ? 0 : inode_number_width), format_inode (buf, sizeof buf, f));
      else
        dired_pos += l1_printp(
            L1_PFORMAT "%*s ",
            (format == with_commas ? 0 : inode_number_width), format_inode (buf, sizeof buf, f));
  ...
  if (print_block_size)
    {
      if (f->stat_ok)
        dired_pos += l1_printp(
            ...
            L1_PFIELD_TEXT "%*s ", // FIXME: separate padding
            ST_NBLOCKS (f->stat),
            format == with_commas ? 0 : block_size_width,
                        ! f->stat_ok ? "?"
  ...
  if (print_scontext)
    dired_pos += l1_printp(
        L1_PFIELD_NAME "security_context"
        // FIXME: seperate padding
        L1_PFIELD_TEXT "%*s ",
        format == with_commas ? 0 : scontext_width, f->scontext
    );

  print_name_with_quoting (f, false, NULL, start_col);
...
{
  if (!ind->len) return; // l1_packet can't be empty
  if (! used_color)
    {
      used_color = true;
  ...
  struct l1_packet p = l1_packet_init(L1_FORMAT, (char *)ind->string, ind->len);
  l1_write(&l1_atrout, &p);
}
      ...
      if (filesno != 0)
        {
          ...
          l1_printf(L1_FORMAT, "%c%c", sep, separator);
        }
...
indent (size_t from, size_t to)
{
  ...
  struct l1_packet p = l1_packet_init(L1_FORMAT, buf, n);
  l1_write(&l1_atrout, &p);
  free(p.msg);
}
...
usage (int status)
{
  if (status != EXIT_SUCCESS)
    ...
    {
      // FIXME: print tab completion spec
